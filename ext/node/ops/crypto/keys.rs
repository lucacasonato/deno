  ...
  pub fn to_public_key(&self) -> AsymmetricPublicKey {
    match self {
      ...
      AsymmetricPrivateKey::X25519(key) => {
        AsymmetricPublicKey::X25519(PublicKey::from(key))
      }
  ...
  pub fn new_asymmetric_private_key_from_js(
    ...
    let private_key = match alg {
      ...
      DH_KEY_AGREEMENT_OID => {
        // FIXME(lucacasonato): implement DH key parsing?
        AsymmetricPrivateKey::Dh(
  ...
  pub fn new_asymmetric_public_key_from_js(
    ...
    let public_key = match spki.algorithm.oid {
      ...
      EC_OID => {
        ...
        match named_curve {
          ID_SECP224R1_OID => {
            let public_key = p224::PublicKey::from_sec1_bytes(
              spki.subject_public_key.as_bytes().unwrap(),
            )?;
          ...
          ID_SECP384R1_OID => {
            let public_key = p384::PublicKey::from_sec1_bytes(
              spki.subject_public_key.as_bytes().unwrap(),
            )?;
    ...
    match typ {
      "pkcs1" => match self {
        ...
        AsymmetricPublicKey::RsaPss(_) => {
          Err(type_error("cannot export RSA-PSS public key as PKCS#1"))
        }
        AsymmetricPublicKey::Dsa(_) => {
          Err(type_error("cannot export DSA public key as PKCS#1"))
        }
        AsymmetricPublicKey::Ec(_) => {
          Err(type_error("cannot export EC public key as PKCS#1"))
        }
        AsymmetricPublicKey::X25519(_) => {
          Err(type_error("cannot export X25519 public key as PKCS#1"))
        }
        AsymmetricPublicKey::Ed25519(_) => {
          Err(type_error("cannot export Ed25519 public key as PKCS#1"))
        }
        AsymmetricPublicKey::Dh(_) => {
          Err(type_error("cannot export DH public key as PKCS#1"))
        }
      },
      "spki" => {
        let der = match self {
...
pub fn op_node_export_public_key_pem(
  ...
  PemWriter::new(type_label, line_ending, out)
}

...
fn build_ops<DBH: DatabaseHandler + 'static>(
  ...
  builder
    .ops(vec![
      ...
      op_state_snapshot_read::decl::<DBH::DB>(),
...
impl TryFrom<KvEntry> for V8KvEntry {
  ...
  fn try_from(entry: KvEntry) -> std::io::Result<Self> {
    Ok(V8KvEntry {
...
#[op]
async fn op_state_snapshot_read<DB>(
  state: Rc<RefCell<OpState>>,
  rid: ResourceId,
  ranges: Vec<(Vec<V8KeyPart>, Option<Vec<V8KeyPart>>, u32)>,
  ...
  DB: Database + 'static,
{
  ...
  let read_ranges = ranges
    ...
    .map(|(start, end, limit)| {
      let start =
        encode_key(&Key(start.into_iter().map(From::from).collect()))?;
      let end = end
        .map(|x| encode_key(&Key(x.into_iter().map(From::from).collect())))
        .transpose()?
        .unwrap_or_else(|| {
...
impl TryFrom<V8KvCheck> for KvCheck {
  type Error = std::io::Error;
  fn try_from(value: V8KvCheck) -> std::io::Result<Self> {
...
impl TryFrom<V8KvMutation> for KvMutation {
  type Error = std::io::Error;
  fn try_from(value: V8KvMutation) -> std::io::Result<Self> {
...
impl TryFrom<V8Enqueue> for Enqueue {
  type Error = std::io::Error;
  fn try_from(value: V8Enqueue) -> std::io::Result<Self> {
    Ok(Enqueue {

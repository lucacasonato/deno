...
async fn compile_command(
  ...
  compressor.write_all(&*bundle_str.as_bytes().to_vec()).expect("expected to write bundle to gzip");
  let compressed = compressor.finish().expect("expected to compress bundle");

  create_standalone_binary(compressed, out_file).await?;
